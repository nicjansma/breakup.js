...
// jsHint directives
//
/*jshint bitwise:true, camelcase:true, curly:true, eqeqeq:true, forin:true, immed:true */
/*jshint indent:4, latedef:true, newcap:true, noempty:true, nonew:true, quotmark:true, jquery:true */
/*jshint undef:true, unused:true, strict:true, trailing:true, browser:true, node:true, white:false */
...
(function() {
     ...
     *
     * @param {Array} array Array (or Array-like object) to iterate over.
     ...
     */
    breakup.forEachSeries = function(arr, iterator, callback, workTime, yieldTime) {
                ...
                if (err) {
                    ...
                    if (current === arr.length) {
                        ...
                        if (now > endTime) {
                            ...
                            if (immediate) {
                                immediate(function() { iterate(); });
                            } else {
                                // if we're not doing an immediate yield, also add yield time
                                endTime += yieldTime;

                                setTimeout(function() { iterate(); }, yieldTime);
                            }
                        } else {
     ...
     *        (optional, defaults to DEFAULT_YIELD_TIME).
     */
    breakup.each = function(arr, iterator, callback, workTime, yieldTime) {
